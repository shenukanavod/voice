...
try:
    ...
    db_logger = setup_database_logging(db_instance, "api", console_output=True)
    logger = db_logger.get_logger()
except Exception as e:
    # Fallback to standard logging if database connection fails
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.warning("Falling back to standard logging only")
    db_instance = None
    db_logger = None
...
def log_verification_attempt(user_id: str, verified: bool, similarity: Optional[float], 
    ...
    """Log verification attempt to file and database."""
    log_entry = {
    ...
    if db_instance:
        try:
            ...
            message = f"Verification {'succeeded' if verified else 'failed'} for user {user_id}"
            if similarity:
            ...
            logger.error(f"Failed to save verification log to database: {e}")
    ...
    **Example:**
    ...
    try:
        ...
        if is_locked:
            ...
            if db_logger:
                db_logger.warning(f"User locked out, {remaining:.1f}s remaining", user_id=user_id)
            else:
        ...
        # Log with user_id context
        if db_logger:
            db_logger.info(f"Verification request: filename={audio.filename}", user_id=user_id)
        else:
        ...
        if not embedding_file.exists():
            ...
            if db_logger:
                db_logger.warning(error_msg, user_id=user_id)
            else:
        ...
        try:
            ...
            if db_logger:
                db_logger.info("Loaded enrolled embedding", user_id=user_id)
            else:
