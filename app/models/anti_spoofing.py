...
class AntiSpoofingSystem:
    ...
    def detect_spoofing(self, audio: np.ndarray, spectrogram: np.ndarray, sr: int = 16000, mode: str = 'verification') -> Dict[str, any]:
        """
        ...
        Args:
            ...
            sr: Sample rate
            mode: 'enrollment' or 'verification' - determines threshold strictness
        ...
        try:
            ...
            # Since CNN is not trained yet, rely more on spectral and replay detection
            if self.spectral_anomaly.is_trained:
                # Use trained spectral model
                weights = [0.2, 0.5, 0.3]  # Reduced CNN weight, increased spectral
            else:
                # No spectral training, rely on CNN and replay
                weights = [0.6, 0.1, 0.3]
            ...
            # Adaptive thresholds based on what's trained
            if mode == 'enrollment':
                # Very lenient for enrollment (admin supervised)
                threshold = 0.20
            else:
                # Verification: adaptive based on training status
                if self.spectral_anomaly.is_trained:
                    threshold = 0.45  # Stricter when spectral model is trained
                else:
                    threshold = 0.35  # More lenient when only CNN available
            
            results['is_genuine'] = combined_score > threshold
