...
try:
    ...
    db_logger = setup_database_logging(db_instance, "monitoring_gui", console_output=True)
...
class MonitoringWindow:
        ...
        if self.db_instance:
            try:
                self.db_instance.save_log(
                    level="INFO",
                    message=f"Voice monitoring started for {self.user_id}",
                ...
                print(f"⚠️  Failed to log monitoring start: {e}")

    def stop_monitoring(self):
        """Stop voice monitoring."""
        # Get final stats before stopping
        final_status = self.monitor.get_monitoring_status() if self.monitor else None
        ...
        if self.db_instance and final_status:
            try:
                self.db_instance.save_log(
                    level="INFO",
                    message=f"Voice monitoring stopped for {self.user_id}",
                    module="admin_panel_session",
                    user_id=self.user_id,
                    extra_data={
                        "action": "Monitoring Stopped",
                        "total_checks": int(final_status.get('total_checks', 0)),
                        "successful_checks": int(final_status.get('successful_checks', 0)),
                        "failed_checks": int(final_status.get('failed_checks', 0)),
                        "success_rate": float(final_status.get('success_rate', 0)),
                        "status": "Stopped"
                    }
                )
            except Exception as e:
                print(f"⚠️  Failed to log monitoring stop: {e}")
    ...
    def on_alert(self, result):
        ...
        self.log(f"🚨 ALERT: Verification failed!", level="CRITICAL")
        
        # Don't show large alert dialog - only log it
        # Small popup is already shown by VoiceMonitor
        print(f"🚨 {alert_msg}")

    def log(self, message: str, level: str = "INFO"):
        """Add message to activity log and database."""
